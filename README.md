...
- 开始关卡会扣 1 点体力；体力每 5 分钟恢复 1 点，直到恢复到上限；体力恢复和进度保存在 MySQL。
...
猜谜语题库第一版使用项目自建审核库，当前不少于 300 条，覆盖动物、自然、日用品、食物、学习用品、交通、身体等通用生活分类。外部谜语只作为授权明确后的补充来源；例如 CC-Riddle 使用 CC BY-NC-SA 4.0 且限制非商业用途，因此不会直接导入产品题库。不要从普通网页、知乎、论坛等来源不明内容直接抓取上线。
